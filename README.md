...
Рабочее место оператора: `http://localhost:3000/staff`. Для локального запуска
задайте `DEMO_STAFF_PIN` в `.env`; demo-коды сотрудников в каждом отделении —
`operator-1` и `operator-2`. Затем выберите отделение, окно и доступные услуги.
Логика окна и проверяемые переходы описаны в
[docs/staff-workplace.md](docs/staff-workplace.md).
...
- Каталог из пяти демонстрационных отделений, четырёх услуг, поиск по
  шестизначному коду и свободные слоты.
...
- Тестовый QR-стенд с печатью, загрузкой SVG и имитацией сканирования на одном
  устройстве.
- Рабочее место оператора: учётные записи с привязкой к отделению, отзыв сессии,
  окна и услуги, живая очередь, конкурентно безопасный вызов, повторный вызов,
  обслуживание, неявка, возврат, перенаправление и регистрация проблем.
...
потери связи. Интерфейс руководителя отделения остаётся следующим отдельным
контуром; права руководителя не смешиваются с рабочим местом оператора.
Алгоритм QR-входа описан в
